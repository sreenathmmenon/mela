...
- Sreenath rejected both the heavy serif typography and the subsequent subtle Instrument Sans adjustment. The current type system uses self-hosted Outfit headings/game names and DM Sans reading/controls, with open tracking and restrained weights; see `docs/TYPOGRAPHY_REVIEW.md`. Preserve colourful game covers and readable mobile navigation. Typography can be consistent across the product, but game-stage physics, camera geometry and authority are not typography targets. Do not cite passing technical audits as proof that the user likes the design.
