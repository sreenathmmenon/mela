...
Sreenath approved guest-first entry on 2026-09-06, superseding required name/email onboarding. Choosing a game or joining its crowd creates the server-owned profile with a generated, editable nickname; do not put a name, email, password or verification wall before play. Saving across devices is optional and requires a verified-email SpacetimeAuth identity; an unverified contact or anonymous provider login must never recover another profile. Preserve original guest credentials while linking; never silently merge guest and existing saved profiles. Existing private contact records and legacy `@users.invalid` placeholders remain intact: never send to placeholders or count them as email signups/deliveries. New guests do not receive fabricated email records or unsolicited welcome emails. Contact data stays private; email collection is not authentication or marketing consent. Treat SpacetimeDB identity as server-issued, validate all actions server-side, guard abuse, and never persist service secrets in the browser or repository.
...
- On 2026-09-13 Sreenath explicitly authorized revising the whole product experience. Use Play, Watch, Agents and Your Mela as clear destinations; immediate game entry comes before character configuration. Keep the character workshop and external-agent connection distinct and optional. Show only individually supported friend/agent modes, real active rooms and truthful completed memories. Keep concise copy, optional sign-in, accessible responsive controls and actionable errors. Do not use fabricated activity, manipulative retention or guaranteed-superiority claims. Research and rationale: `docs/MELA_WHOLE_PRODUCT_EXPERIENCE.md`.
- Sreenath rejected both the heavy serif typography and the subsequent subtle Instrument Sans adjustment. Use self-hosted Outfit for the wordmark/game-page headings and DM Sans for discovery names, reading and controls; see `docs/TYPOGRAPHY_REVIEW.md`. Play starts directly with a compact mode switch and cards, without a visible “Choose a game” instruction hero; preserve the accessible heading. Preserve colourful covers and readable mobile navigation. Verify typography changes in the user's actual open production tab as well as a separate test browser: an open tab may render an older release. Game-stage physics, camera geometry and authority are not typography targets. Do not cite passing technical audits as proof that the user likes the design.
...
On 2026-09-15 playable recorded-moment practice and saved character editions were implemented; see `docs/MELA_V3_PLAYABLE_MEMORIES.md`. Preserve original history, versioned server-owned checkpoints, fresh private crowd state, declared deterministic practice opponent, no practice progression farming, immutable owner-only saved editions and caller-scoped roster views. Saved characters are not delegated external-agent accounts. Keep practice results distinct from full matches, and retain saved edition references on owner rematches. Do not claim the remaining v3 roadmap or competitive superiority is complete.

On 2026-09-13 Sreenath requested a working human/agent gaming playground. The three newer arenas now have independently owned `arena_room` seats for friends, human-agent and agent-agent modes; see `docs/ARENA_INDEPENDENT_SEATS.md`. Preserve private simultaneous intents, human reconnect waits, distinct crowd links vs private human invitation capabilities, and explicit missed-agent substitutions. Private arena energy must not leak pending power costs to players through public pool updates. External agents are ordinary reducer clients; self-reported model names are not attestation. Do not describe two-seat games as four-player games, process-local MCP sessions as persistent agent accounts, or scripted transport QA as LLM inference.
...
Sreenath approved human-v-human, human-v-agent and watchable agent-v-agent modes, starting with Pen Fight and extending one tested game at a time. Four in a Row is the second implementation. See `docs/MULTIPLAYER_SEATS.md`: canonical-identity seat ownership, private player invitation vs public crowd link, no automatic bot replacement of human turns, and durable disclosure of missed-agent substitutions. Preserve normal solo modes and per-game rules; do not silently call scripted fallback an external agent or mix these unranked modes into solo skill ratings. Build repeat play through fair competition, readable actions, spectators and memory—not fabricated activity or manipulative retention.
